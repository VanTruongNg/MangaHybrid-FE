...
import { UploadMangaDialog } from "./manga/upload-manga-dialog";
...
export function UserNav({ className }: UserNavProps) {
  ...
  const { isReaderPage, mounted } = useReaderRoute();
  const setUploadMangaOpen = useUploadMangaStore((state) => state.setOpen);
  ...
  if (!mounted || isReaderPage) return null;
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent className="w-56" align="end" forceMount>
              ...
              {(user.role === "admin" || user.role === "uploader") && (
                <>
                  <DropdownMenuItem onSelect={() => setUploadMangaOpen(true)}>
                    Đăng truyện
                  </DropdownMenuItem>
                  <DropdownMenuItem asChild>
                    <Link
                      href="/manga/my-uploads"
